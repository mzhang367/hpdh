...
class CIFAR10(data.Dataset):
    ...
    def __init__(self, root, train=True,
        ...
        self.root = os.path.expanduser(root)    # here, change the directory to the ssd/data
        ...
        if self.train:
            ...
            self.labels = np.concatenate([np.array(self.train_labels).reshape(-1, 1), np.array(self.train_coarse_labels).reshape(-1, 1)], axis=1)
            ...
            self.labels = np.concatenate([np.array(self.test_labels).reshape(-1, 1), np.array(self.test_coarse_labels).reshape(-1, 1)], axis=1)
